...
\end{lemma}
We leave the proof as an exercise (see also \ocite{mac-lane}*{Section IV.1}).
...
and the proofs are similar (see \ocite{lurie-cobordism}*{Lemma 2.3.8}).
...
\ocite{pstragowski}, we will impose the following relations.
...
property). We refer the reader to \ocite{pstragowski}*{Theorem 2.7} for
...
can be found in \ocite{pstragowski}*{Section 2.2}.
...
in \ocite{pstragowski}*{Theorem 2.14} and says that the 2-groupoid of
...
It is not difficult to show (see \ocite{lurie-cobordism}*{Proposition
  4.2.3}) that an object $A$ is fully dualizable if and only if $A$ has a
...
\begin{remark}
  ...
  \ocite{lurie-cobordism}*{Remark 4.2.4}.
\end{remark}
...
\begin{theorem}\label{t:serre}
  ...
  $S_A\colon A\to A$ be the Serre morphism defined by \deref{d:serre}.
   ...
   \end{align*}
\end{theorem}
We refer the reader to \ocite{pstragowski}*{Section 3.1} for the proof.
...
\end{theorem}
\begin{proof}
  Our proof follows \ocite{bdspv}*{Section A.6}, where the proof is attributed
...
\ocite{etingof-fusion}*{Section 1.8}.
...
\end{theorem}
A proof of this theorem can be found in \ocite{dsps-balanced}*{Proposition 1.7}
