...
%References: \ocite{dsps-balanced}, \ocite{dsps-dualizable},
% \ocite{etingof-fusion}.
...
the works \ocite{dsps-balanced}, \ocite{dsps-dualizable}; the construction of
...
\ocite{dsps-balanced} do not include this condition). But the rigidity condition
...
\begin{lemma}\label{l:tensor-biexact}
  ...
  \end{enumerate}
\end{lemma}
A proof of this lemma can be found in \ocite{etingof-fusion}*{Proposition
4.2.1, Theorem 4.3.8}.
...
\end{definition}
Note that in many references (e.g., \ocite{dsps-balanced}), condition that
...
\end{theorem}
We refer the reader to \ocite{dsps-balanced}*{Corollary 2.26} for the proof.
...
\begin{example}\label{x:algebra-in-cat}
  ...
  associativity and unit axioms (see \ocite{etingof-fusion}*{Section 7.8}). A
  ...
  \ocite{etingof-fusion}*{Corollary 7.10.5}) that every finite $\calC$ module
...
% \ocite{dsps-balanced}*{Lemma 2.10}.
%\end{remark}
...
\ocite{etingof-fusion}*{Section 7.2}). This turns $\Fun_\calC(\calM, \calN)$
...
\end{theorem}
Proof of the first part can be found in \ocite{etingof-fusion}*{Proposition
7.11.6}. (Note that in the book, they did not spell out exact requirements on
the module categories; however, if you read the proof, you will see that it
works for any finite module categories).

The second part is \ocite{eno}*{Theorem 2.16}.
...
\begin{definition}\label{d:balanced}
  ...
  $$
  satisfying usual axioms (see \ocite{dsps-balanced}*{Definition 3.1}).
\end{definition}
...
\end{theorem}
A proof of this theorem can be found in \ocite{dsps-balanced}*{Theorem~3.3}.
...
\begin{lemma}\label{l:bimodule-composition}
  ...
  bimodule category.
\end{lemma}
This lemma can be found in \ocite{dsps-balanced}*{Remark 3.7}.
...
\ocite{dsps-dualizable}*{Theorem 2.2.18}.

\begin{remark}
  In my opinion, the current state of this and related statements is somewhat
  unsatisfactory. \ocite{dsps-dualizable}*{Theorem 2.2.18} does not give a
  proof; instead, they refer the reader to \ocite{jfs}, where this statement
...
Note that \ocite{jfs} use a different model of higher categories than we do
