...
def post_comment():
  # The tag comes from the form's hidden field (always present). We used to read
  # it from the Referer header, but some browsers/privacy settings omit Referer,
  # which crashed this view with a KeyError. The captcha is still check == tag.
  tag = request.form["tag"]

  if tag == request.form["check"]:
