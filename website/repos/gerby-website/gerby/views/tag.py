...
def show_tag(tag):
  ...
  if tag.type in headings and headings.index(tag.type) < headings.index(gerby.configuration.UNIT):
    ...
    if tag.type == "part":
      ...
      tags = (Tag.select(Tag,
                         ...
                         Slogan.html,
                         History.html)
                .where(Tag.ref.startswith(tag.ref + "."), Tag.type << headings)
                .join_from(Tag, Slogan, JOIN.LEFT_OUTER, attr="slogan")
                .join_from(Tag, History, JOIN.LEFT_OUTER, attr="history")
                .join_from(Tag, Reference, JOIN.LEFT_OUTER, attr="reference"))
  ...
  # Per-tag PDF-download filename. The original assumes Stacks label-naming
  # conventions (e.g. "...-part-N"); guard so arbitrary labels don't crash.
  try:
    if tag.type == "part":
