<h2 id="s:cat-basics" class="tex-section"><span data-tag="0001">1.1</span> Category theory basics</h2>
<p>
...
One special case of a category, which we will use frequently, is a groupoid. <article class="env-definition" id="">
  <p><a class="environment-identifier" href="/tag/">Definition <span data-tag="">1.1.1</span>.</a> A groupoid is a category in which all morphisms are invertible. </p>
</article> <article class="env-example" id="">
  <p><a class="environment-identifier" href="/tag/">Example <span data-tag="">1.1.1</span>.</a>Let $X$ be a topological space. Define its <em>fundamental groupoid</em> $\pi (X)$ as the category whose objects are points in $X$ and morphisms are homotopy equivalence classes of paths between points, with the obvious composition. </p>
</article> 
</p>
<p>
...
<p>
The common way to resolve this is the following convention. <article class="env-convention" id="0002">
...
<p>
Formally, this convention can be justified by constructing a category $\widehat\mathcal{C}$, whose objects are such collections $(X_\alpha , f_{\alpha \beta })$, and observing that categories $\mathcal{C}$, $\widehat\mathcal{C}$ are equivalent; see, e.g., [bakalov-kirillov]*Lemma 1.1.12. 
</p>
