...
<p>
 As is common in the physics literature, we will assume that the field $\varphi $ is smooth enough for all purposes. We denote by $\mathcal{F}_ N$ the space of all fields (at a fixed moment of time); in the simplest example above, $\mathcal{F}_ N=C^\infty (N,V)$. This space is the analog of the configuration space $N$ in classical mechanics. 
</p>
<p>
The analog of a trajectory is now a field $\varphi (x,t)$ on spacetime $M = N \times [t_0,t_1]$. If the space $N$ is $(n-1)$-dimensional, then spacetime $M$ is $n$-dimensional; the corresponding field theory is referred to as an $n$-dimensional field theory. 
</p>
<p>
Note that the space $N$ plays a different role here than in mechanics. In a field theory, $N$ is the domain of the field $\varphi $, whereas in mechanics, the configuration space $N$ was the target of the “position field” $x$. From this perspective, we can think of the discussion in the previous section as a $(0+1)$-dimensional field theory of the position field $x\colon \text{point} \to N$. 
</p>
<p>
...
<p>
 Here $\mathcal{L}$ is called the Lagrangian density; it depends on the value of the field $\varphi $ at the point $(x,t)$, its first derivatives $\partial _\mu \varphi $, and the metric $g$ on spacetime. 
</p>
<p>
The classical equations of motion again follow from the stationary action principle: $\delta S[\varphi ]=0$. This is equivalent to a system of PDEs on $\varphi $, called the <em>Euler–Lagrange equations</em>. 
</p>
<p>
Similar to how we moved from classical to quantum mechanics, it is natural to expect that in quantum field theory, the state of the system is described by a functional $\psi \in \mathcal{H}_ N = L^2(\mathcal{F}_ N)$. The space $\mathcal{F}_ N$ is infinite-dimensional, and there is no canonical Lebesgue measure that we can define for it, so the exact meaning of $L^2$ is unclear; for the moment, let us ignore that. 
</p>
<p>
...
<p>
 Here $\Phi =\Phi (x,t)$ is a field on spacetime $M=N\times [t_0,t_1]$ with given values at $t=t_0$, $t=t_1$; it is the analog of the path $\gamma $ in mechanics, and $D\Phi $ is some (not yet defined) measure on the space of fields. 
</p>
<p>
Note that the integral in (<a href="/tag/00FP" data-tag="00FP">1.2.2</a>) does not explicitly use the decomposition of spacetime $M$ as a product of space $N$ and time interval $[t_0, t_1]$: it only uses the metric on $M$ (which is used in the Lagrangian density) and the fact that $M$ has two boundary components, $N_0=N\times \{ t_0\} $ and $N_1 = N\times \{ t_1\} $. The same integral makes perfect sense if we allow spacetime $M$ to have a different topology: for any $n$-dimensional manifold $M$ whose boundary is presented as $\partial M=\overline{N_0}\sqcup N_1$ (where the bar denotes orientation reversal), formula (<a href="/tag/00FP" data-tag="00FP">1.2.2</a>) defines an operator $U\colon \mathcal{H}_{N_0}\to \mathcal{H}_{N_1}$. 
</p>
<p>
...
<p>
However, if we are willing to ignore these problems for now, we can use the path integral formula (<a href="/tag/00FP" data-tag="00FP">1.2.2</a>) to establish various properties of QFT. For example, it is easy to show the composition property 
</p>
<div class="equation">
  \begin{equation*}  U_{t_0,t_2}= U_{t_1,t_2} U_{t_0,t_1}. \end{equation*}
</div>
<p>
 Indeed, since a field $\Phi \in \mathcal{F}_{N\times [t_0,t_2]}$ is the same as a pair of fields $\Phi _1\in \mathcal{F}_{N\times [t_0,t_1]}$, $\Phi _2\in \mathcal{F}_{N\times [t_1,t_2]}$ whose values at the common boundary $N\times \{ t_1\} $ coincide, and $S[\Phi ]=S[\Phi _1]+S[\Phi _2]$, we get 
</p>
...
<p>
 where $M=N\times [t_0,t_2]$, $M_1=N\times [t_0,t_1]$, and $M_2=N\times [t_1,t_2]$. 
</p>
<p>
More generally, the same (non-rigorous) argument shows that for an arbitrary spacetime $M$, we can cut $M$ along a codimension-one submanifold $N$ and split the path integral into integrals over the two resulting pieces, which share $N$ as a common boundary. 
</p>
<p>
