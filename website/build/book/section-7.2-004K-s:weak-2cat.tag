...
As a variation of the above, we can define a weak 2-category $\mathbf{Ab}_\mathbf{k}$ of $\mathbf{k}$-linear abelian categories, together with $\mathbf{k}$-linear functors between them. We can also impose some additional restrictions, such as local finiteness (all $\operatorname{Hom}$ spaces are finite-dimensional, and every object has finite length) or semisimplicity. 
  ...
  <p><a class="environment-identifier" href="/tag/004N">Example <span data-tag="004N">7.2.2</span>.</a> Let $\mathcal{C}$ be a monoidal category. Define the 2-category $\mathcal{B}\mathcal{C}$ as the 2-category with one object $*$ and morphisms given by $\operatorname{Hom}_{\mathcal{B}\mathcal{C}}(*,*) =\mathcal{C}$, with horizontal composition given by tensor product. It is easy to see that this is indeed a 2-category; conversely, every 2-category with only one object is obtained in this way. This category is called the <em>delooping</em> of $\mathcal{C}$. Moreover, given monoidal categories $\mathcal{C}$, $\mathcal{D}$, a monoidal functor $F\colon \mathcal{C}\to \mathcal{D}$ is the same as a weak functor between weak 2-categories $F\colon \mathcal{B}\mathcal{C}\to \mathcal{B}\mathcal{D}$, see <span class="cite">[<a href="/bibliography/leinster">Lei2004</a>, <span class="postnote">Example 1.5.9</span>]</span>. </p>
</article> 
</p>
<p>
<article class="env-remark" id="004P">
  ...
  <p>The relation $\mathcal{B}G\simeq \Pi _1(BG)$ above is just the beginning of a chain of relations. For example, let $G$ be a topological group (which we assume to be nice enough, e.g. having the topology of a CW complex); by Example&#160;<a href="/tag/00FX" data-tag="00FX">2.3.2</a>, its fundamental groupoid $\Pi _1(G)$ is a monoidal category, so we can define its delooping $\mathcal{B}(\Pi _1(G))$ which is a weak 2-category. Then we have an equivalence of weak 2-categories </p>
  <div class="equation" >
    <span class="equation-label">7.2.1</span>
    <div>\begin{equation}  \mathcal{B}(\Pi _1(G))\cong \Pi _{\le 2}(BG) \end{equation}</div>
</div>
  <p> where $BG$ is the classifying space, and $\Pi _{\le 2}$ is the fundamental 2-groupoid (see Example&#160;<a href="/tag/004E" data-tag="004E">7.1.3</a>). </p>
  <p>This statement has a natural generalization for any $k$, which we will discuss in FIXME. </p>
</article> 
</p>
<p>
