...
<p>
So far we have not specified what we mean by “manifold”. Usually in physics the spacetime is an oriented pseudo-Riemannian manifold, with a metric of signature $(1, n-1)$. However, we will be interested in a very special case, namely when all the structures above do not use the metric on spacetime; thus, we only require $M$ to be an oriented smooth manifold. Such theories are called <em>topological</em>. The most famous example of such a theory is Chern–Simons theory<a class="footnotemark" href="#a0000001328" id="a0000001328-mark"><sup>1</sup></a> (see <span class="cite">[<a href="/bibliography/witten-jones">Wit1989</a>]</span>), in which the spacetime $M$ must be 3-dimensional, and fields are $\mathrm{SU}(r)$ connections on $M$. After choosing a trivialization of the bundle, these fields can be described by one-forms, usually denoted by $A$, with values in the Lie algebra $\mathfrak {su}(r)$. The action is then given by 
</p>
<div class="equation" >
