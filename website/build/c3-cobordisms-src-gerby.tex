...
TQFT, following the work of Atiyah (see \cite{atiyah}). To do that, we first
define the notion of a cobordism.
...
orientation-preserving smooth map with a smooth inverse.
...
The picture below shows a 2-dimensional cobordism between a circle and a
disjoint union of two circles; this cobordism is usually called the \emph{pair
of pants}\index{pair of pants}.
\begin{center}
\includegraphics{figures/c3-fig01.svg}
\end{center}

\begin{convention}\label{conv:cobordism-pictures}
...
\begin{definition}\label{d:cobordism-isomorphism}\index{cobordism!isomorphism}
  An isomorphism of two cobordisms $M_1, M_2$ from $N_0$ to $N_1$ is a
  diffeomorphism $f\colon M_1\to M_2$ which makes the following diagram commute:
  \begin{equation}\label{e:cob-iso}\includegraphics{figures/c3-fig02.svg}\end{equation}
\end{definition}
Given two cobordisms $N_0\cob{M_1}N_1$, $N_1\cob{M_2}N_2$, it is natural to
ask if they can be glued together into a single cobordism $M=M_2\circ M_1$
$$
...
\begin{itemize}
  ...
  \item One can consider \emph{collared}\index{collared manifold}
    ...
    an isomorphism $\ov{N_0}\sqcup N_1\to \del M$,
    we can require in the definition of a cobordism an isomorphism of
...
\end{theorem}
For a proof, see \cite[Theorem 1.3.12]{kock}.
...
\end{definition}
Similarly one defines the PL version of the cobordism category.
...
\begin{theorem}\label{t:1d-TQFT}
  A 1-dimensional TQFT is equivalent to the following collection of data:
  \begin{itemize}
  ...
  \begin{equation}\label{e:1d-tqft-rigidity}\includegraphics{figures/c3-eqfig01.svg}\end{equation}
\end{theorem}
\begin{proof}
  Since $Z$ is symmetric monoidal,
  $Z(N_1\sqcup N_2)\simeq Z(N_1)\otimes Z(N_2)$, so $Z$ on objects is determined
  by its values on connected 0-manifolds. The only connected 0-manifolds are the
  positively and negatively oriented points $\bullet^+, \bullet^-$, hence $Z$ on
  objects is determined by $V_\pm := Z(\bullet^\pm)$.
...
Comparing this with the definition of a dual pair given in \seref{s:rigid}, we
see that this data is exactly the data of a dual pair $(V_+, V_-)$. Therefore,
by \leref{l:rigidity_vect}, $V_+$ and $V_-$ must be finite-dimensional, and
$V_-\simeq (V_+)^*$, so we get the following result.

\begin{theorem}\label{t:1d-tqft2}
  A 1-dimensional TQFT is completely determined by the vector space
  $V=Z(\bullet^+)$, which must be \emph{finite-dimensional}. Conversely, every
  finite-dimensional vector space $V$ uniquely defines a 1-dimensional TQFT. For
...
\begin{theorem}\label{t:1d-tqft}
  ...
  a 1-dimensional TQFT such that $Z(\bullet^+)=A$.
\end{theorem}
...
Let $Z$ be an $n$-dimensional TQFT with values in $\vect$, and let $M$ be an
$n$-dimensional manifold with boundary. Let $N\subset M$ be a closed smooth
codimension-one submanifold contained in the interior of $M$, and denote by
$M_{\text{cut}}$ the manifold with boundary
...
\begin{figure}[ht]
  ...
  \includegraphics{figures/c3-fig03.svg}

  \caption{Cutting a manifold $M$ along a codimension-one submanifold $N$.}
  \label{f:cutting}
\end{figure}


This lemma is another form of the gluing axiom for TQFT (compare with
\eqref{e:tqft-gluing0}). We leave the proof as an exercise to the reader. 
...
$Z(\ph)\colon Z(N_0)\to Z(N_1)$; however, we did not include that as part of
the definition. It turns out that it is not necessary.
...
\begin{lemma}\label{l:tqft-isotopy}
  Let $C_\ph$ be as defined above. Then:
  \begin{enumerate}
  ...
  \end{enumerate}
\end{lemma}
Recall that $\ph, \ph'$ are called isotopic if there exists a smooth
one-parameter family $\ph_t$, $t\in[0,1]$, of smooth maps $N_0\to N_1$ such that
$\ph_t$ is a diffeomorphism for all $t$, and $\ph_0=\ph$, $\ph_1=\ph'$.

We leave the proof of this lemma as an easy exercise to the reader.

\begin{remark}\label{r:pseudo-isotopy}
...
\begin{theorem}\label{t:tqft-functoriality}
  ...
  \begin{enumerate}
    \item For any orientation-preserving diffeomorphism $\ph\colon N_0\to N_1$,
      define $Z(\ph)\colon Z(N_0)\to Z(N_1)$ by $Z(\ph)=Z(C_\ph)$. Then
      $Z(\ph_2\ph_1)= Z(\ph_2)\circ Z(\ph_1)$.
    \item If $\ph,\ph'$ are isotopic, then $Z(\ph)=Z(\ph')$.
  \end{enumerate}
\end{theorem}

As a corollary, this gives the action of the mapping class group on $Z(N)$.
Recall that for an oriented  closed manifold $N$, the mapping clas group
\index{mapping class group} $\Ga(N)$ is defined as the group of isotopy
classes of orientation-preserving diffeomorphisms $\ph \colon N\to N$. 



\begin{corollary}\label{c:mapping-class}
  Let $Z$ be an $n$-dimensional TQFT. Then for any closed  oriented $(n-1)$-manifold 
  $N$,  vector space $Z(N)$ has a natural action of the mapping class group $\Ga(N)$. 
\end{corollary}  
