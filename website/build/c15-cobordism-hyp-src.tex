...
In this chapter, we give a definition of fully extended TQFT in
any dimension, using the language of $(\infty,n)$ categories, and
present one of the key results of this theory, the cobordism hypothesis,
which classifies such $n$-dimensional extended theories in terms of
\emph{fully dualizable} objects.

Our exposition follows the foundational paper
\cite{lurie-cobordism} by Jacob Lurie; a short exposition can also be
found in \cite{freed-cobordism}.
...
conjecture by Baez and Dolan \cite{baez-dolan-tqft} in 1995. In 2008, Jacob
Lurie published a famous paper \cite{lurie-cobordism}, where he  restated this
...
\cite{igusa}. Since then, many people worked on filling the details of the
proof and extending it to other situations, such as \cite{ayala-francis2017}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$SO(n)$ action and oriented cobordism hypothesis}
\label{s:oriented-cobordism-hypothesis}
...
\begin{example}\label{x:group-action-1cat}
  ...
  \cite[Section 2.7]{etingof-fusion}.
...
%(not necessarily discrete) group $G$ on a 2-category and definition of
%$\calC^G$ in this case. Unfortunately, this gives a much longer list of
%consistency conditions; one can find all details in \cite{hesse}.


If we go to arbitrary dimension and want to define topological group actions on
$n$-categories (or $(\infty,n)$ categories), writing it algebraically is not an
option: we must use topology of the group. Informally, action of $G$ on the
category $\calC$ should be decribed by as follows (borrowed from
\cite[Section 16]{schommer-pries-category})

\begin{itemize}
  \item Every element $g\in G$ should define an invertible functor
    $\rho(g)\colon \calC\to \calC$, i.e., a point in $\Aut(\calC)$
  \item A path $\ga$ in $G$, connecting $g_1$ and $g_2$, should define
    an isomorphism of functors $\rho(g_1)\to \rho(g_2)$, or equivalently, an
    invertible 1-morphism in $\Aut(\calC)$
  \item \dots
\end{itemize}
This data has to satisfy consistency relations: for example, one should require
existence of functorial isomorphisms $\rho(g_1g_2)\simeq \rho(g_1)\rho(g_2)$,
which themselves should satisfy consistency relations.

The easiest way to state all the consistency relations is by requiring that
$\rho$ is a monoidal functor between monoidal $(\infty, n)$-categories
$$
  \rho\colon \Pi_\infty(G)\to \Aut(\calC)
$$
Equivalently, using FIXME, we can rewrite it as weak functor between
$\infty$-categories
$$
\rho\colon \calB(\Pi_\infty(G) )\to \calB(\Aut(\calC))
$$
or as (topological) map $BG\to B(\Aut(\calC))$
(this is the point of view taken in \cite{schommer-pries-category}).


\begin{remark}\label{r:Ainfty}
  For comparison, it is known that  for  topological groups $G,H$,
  pointed maps $BG\to BH$ correspond to $A_\infty$ morphisms $G\to H$, see
  \cite[Propositions 1.15, 1.16]{boardman-vogt}.
\end{remark}


For $n=2$, one can unpack this definition and write all the required data
and consistency relations explicitly; see \cite[Remark 2.16]{hesse}.
For $n=3$, see \cite{schommer-pries-category}.



In the case of interest to us, there is a shortcut.  Namely, we are only
interested in the action of a group on the $(\infty,0)$ category $\cfd$. By
FIXME, every $(\infty, 0)$ category is a fundamental groupoid of a topological
space: $\calC=\pi_{\le \infty}(X)$. Thus, we can instead  define the action of
$G$ on $\pi_{\le \infty}(X)$ as the action induced by an action of $G$ on $X$.
...
\begin{definition}\label{d:homotopy-fixed}
  Let $X$ be a topological space with a continuous action of a topological
  group $G$. The \emph{homotopy fixed point set}\index{homotopy fixed point} is
  defined by
  $$
    X^{hG} =\Hom_G(EG, X)
  $$
...
details can be found in \cite{lurie-cobordism}.
