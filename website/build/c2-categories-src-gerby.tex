...
referring the reader to books \cite{mac-lane}, \cite{etingof-fusion}
for details.
...
\begin{itemize}
  ...
  \item $\plman_n$: category of piecewise-linear (PL) manifolds of dimension $n$.
     Note that
...
\begin{example}\label{x:fundamental-groupoid}
  ...
  $\Pi_1(X)$ as the category whose objects are points in $X$ and morphisms
  are homotopy classes of paths between points, with the obvious
  composition.
\end{example}
...
\cite[Lemma 1.1.12]{bakalov-kirillov}.


%%%%%%%%%%%%%%
\section{Abelian categories}\label{auto:c2-categories-abelian-categories}
\begin{definition}\label{d:abelian}\index{abelian category}
  ...
  properties (see \cite[Section~1.3]{etingof-fusion} for details).
\end{definition}
...
Unless explicitly stated otherwise,  functors between abelian categories are assumed to be additive,
and functors between $\kk$-linear categories are in addiiton assumet to be $\kk$-linear on morphisms.


\begin{example}\label{x:linear-abelian-cats}
  Below are some examples of $\kk$-linear abelian categories. In all cases, the morphisms are trivial to define.
  \begin{enumerate}
    \item $\vect_\kk$;
...
$\kk$ is $\kk$ itself (see, e.g., \cite[Lemma 3.5]{pierce}).
...
\cite[Chapter 2]{etingof-fusion}; \cite[Section 1.2]{leinster} for a more
in-depth discussion.
...
\begin{theorem}\label{t:coherence-monoids}
  ...
  expressions obtained by writing $a_1, \dots, a_n$ \textup{(}in this order\textup{)}, then
  ...
  \textup{(}with the same $a_1, \dots, a_n$\textup{)}, then $X_1=X_2$.
\end{theorem}
...
\begin{center}
  %%%%% ((12)3) 4
  \includegraphics{figures/c2-fig01.svg}
  \qquad
  %%%%% ((12)(3 4)
  \includegraphics{figures/c2-fig02.svg}
\end{center}
...
\begin{definition}\label{d:monoidal}\index{monoidal category}
  A \emph{monoidal category} is a category $\calC$ together with the following data:
  \begin{itemize}
  ...
  \begin{enumerate}
    \item Triangle axiom: for any $A,B\in \calC$, the diagram below commutes
      \begin{equation}\label{e:triangle}\includegraphics{figures/c2-fig03.svg}\end{equation}
    \item Pentagon axiom: for any $A,B,C,D \in \calC$, the diagram below
      commutes
      \begin{equation}\label{e:pentagon}\includegraphics{figures/c2-fig04.svg}\end{equation}
...
\begin{example}\label{x:fund-group}
  ...
  as defined in \deref{x:fundamental-groupoid}. Then multiplication on $G$
...
\begin{definition}\label{d:monoidal_functor}\index{monoidal functor}
  ...
  \end{align*}
  satisfying suitable constraints (see \cite[Section 2.4]{etingof-fusion}).
...
\begin{remark}\label{r:lax}
  ...
  instead of requiring $J$ to be an isomorphism, one requires $J$ to be a
  functorial morphism (not necessarily invertible). This gives the definition
  of \emph{lax monoidal functor}, see \cite[Definition 1.2.10]{leinster}.
\end{remark}
...
\begin{exercise}\label{ec:twisted-group-cat}
  ...
  \cite[Section 2.6]{etingof-fusion}).
\end{exercise}
...
\begin{figure}[h]
  \centering
  \includegraphics{figures/c2-fig05.svg}
...
\begin{definition}\label{d:symmetric}\index{monoidal category!symmetric}
  ...
  \begin{equation}\label{e:hexagon1}\includegraphics{figures/c2-fig06.svg}\end{equation}
\end{definition}
...
one below
\begin{equation}\label{e:hexagon3}\includegraphics{figures/c2-fig07.svg}\end{equation}
...
called \emph{permutoassociahedron}. (Yes, we know, it is long and
impossible to spell.)


As before, we skip the proof of this theorem, referring the reader to
\cite[Section XI.1]{mac-lane}.
...
In many monoidal categories, cuch as the category of finite-dimensional vector
spaces or the category of finitd-eimensional representations of a group $G$, in
addition ot the tensor product we also have the operation of taking duals: for
...
\begin{equation}\label{e:ev-coev-graphical}\includegraphics{figures/c2-fig08.svg}\end{equation}

Then the rigidity equations \eqref{e:rigidity} are illustrated by
\begin{equation}\label{e:rigidity2}\includegraphics{figures/c2-eqfig01.svg}\end{equation}
...
\begin{example}\label{x:dual-vector}
  ...
  \begin{equation}\label{e:dual-vector}
      ...
      \end{aligned}
  \end{equation}
  where $v_i, v^i$ are dual bases in $V, V^*$ respectively. Then so defined
  $\ev$ and $\coev$ satisfy rigidity equations  \eqref{e:rigidity}; thus,
  $(V^*, V, \ev, \coev)$ for a dual pair.

  Indeed, one easily sees that the rigidiry equations are equivalent to
...
\begin{lemma}\label{l:rigidity_vect}
  A vector space $V$, considered as an object in monoidal category $\vect$, has
  a left dual if and only if it is finite-dimensional. In this case, the left
  dual is given by $V^*$, with evaluation and coevaluation defined as in
  \exref{x:dual-vector}
\end{lemma}
...
\begin{exercise}\label{ec:dual-morphism}
  ...
  \begin{equation}\label{e:dual-morphism}\includegraphics{figures/c2-fig09.svg}\end{equation}
