...
In the previous chapter, we showed that 2-dimensional TQFTs are classified
...
algebra is a vector space $A$ over a field $\kk$ with two structures (see
\deref{d:frobenius}):
\begin{itemize}
...
\begin{remark}\label{r:bialgebra-vs-frobenius}
  ...
  \begin{itemize}
    \item \emph{Bialgebra condition}:
      the comultiplication and the counit are morphisms of algebras.
      (Frequently one also requires the existence of a certain linear map
      $S\colon A\to A$, called
...
the graphs used to represent common algebraic operations.
\begin{figure}[ht]
...
\begin{equation}\label{e:frobenius-graphical}\includegraphics{figures/c5-eqfig01.svg}\end{equation}
...
\begin{equation}\label{e:fr-cap-cup}\includegraphics{figures/c5-eqfig02.svg}\end{equation}
\begin{proof}
  ...
  \begin{equation}\label{e:frob-rigidity}\includegraphics{figures/c5-eqfig03.svg}\end{equation}
...
relation that
\begin{equation}\label{e:Delta}\includegraphics{figures/c5-eqfig04.svg}\end{equation}
...
\begin{exercise}\label{ec:frobenius-dual-basis}
  ...
  \begin{enumerate}
    \item Define the map $\la\colon A\otimes A \otimes A \to \kk$ by
      ...
      \begin{equation}\includegraphics{figures/c5-eqfig05.svg}\end{equation}
    ...
    \item
      ...
      \begin{equation}\label{e:fr-comultiplication}\includegraphics{figures/c5-fig02.svg}\end{equation}
    or, equivalently, by
    \begin{equation}
      (a\otimes b, \De(c))=(ab,c),
    \end{equation}
    where we extend the pairing $(\, , \,)$ to $A\otimes A$ by
    $(a_1\otimes a_2, b_2\otimes b_1)=(a_1, b_1)\cdot (a_2, b_2)$
    (note the order!).
  \end{enumerate}
\end{exercise}
...
\end{corollary}
Indeed, in this case the pairing is symmetric, so $e=\sum x_i \otimes x^i$
is also symmetric, which by \eqref{e:Delta} implies cocommutativity.


Conversely, one can recover the structure of a Frobenius algebra from the
pairing $(\, , \,)$.
...
\begin{equation}\label{e:e-central-graphical}\includegraphics{figures/c5-fig03.svg}\end{equation}
...
however, \exref{x:frobenius2} below shows that a Frobenius algebra can be
...
\begin{lemma}\label{l:bicentral}
  ...
  \begin{enumerate}
    \item $e$ is symmetric: $P(e)=e$, where $P\colon A\otimes A\to A\otimes
    A\colon a\otimes b\mapsto b\otimes a$.
...
Recall that an associative algebra is semisimple if it is semisimple
as a left module over itself. As we will see later, semisimple
algebras play a special role in the
...
closed field, direct sums of such algebras are the only examples of semisimple
symmetric Frobenius algebras. Namely, we have the following lemma.
...
$$
In particular, it implies that for the corresponding TQFT $Z_A$, we have
\begin{align*}
  Z_A(S^2)&=\eps(1)=\sum \la_i\\
  Z_A(T)&=\dim A
\end{align*}
...
\begin{remark}\label{r:euler-element}
  ...
  element is sometimes called the \emph{Euler element}\index{Euler element} and
  will play an important role later; see \thref{t:separable-frobenius}. (In
  \cite{lauda-pfeiffer}, $w$ is called the ``window element''.) Note also that
  it easily follows from the centrality of $e$ that $w$ is central.
\end{remark}
...
\begin{exercise}\label{ec:dual-module-frob}
  ...
  \begin{align*}
    (b\la) (m) & = \la (mb),\qquad \la\in M^*, m\in M\\
    (\la a) (m) &= \la (am).
  \end{align*}
