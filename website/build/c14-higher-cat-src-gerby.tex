...
\cite{grothendieck}. He also proposed the following hypothesis.
...
\cite{gordon-power-street}), but it is so long that I was never able to get to
the end of it; for $n\ge 4$, the algebraic definitions are even more cumbersome.
Some summary of algebraic approaches is collected in \cite{leinster}. On the
...
\cite{toen}, \cite{bsp}, and the book \cite{riehl-verity}; for a short
introduction, there is also Emily Riehl's article \cite{riehl-notices} in
...
exposition follows the paper \cite{schommer-pries-category}; a detailed exposition of
the theory of Segal categories as a model for $(\infty, n)$ categories can be
found in \cite{simpson}.
...
\begin{equation}\label{e:pullback}\includegraphics{figures/c14-fig01.svg}\end{equation}
...
\begin{center}\includegraphics{figures/c14-eqfig01.svg}\end{center}
...
review that, referring the reader to the books \cite{may},
\cite{goerss-jardine} for details.
...
\cite[Chapter III]{may}. In particular, $|K|$ is always a CW complex.
...
$$
(\cite[\S 15]{may}). The unit and counit of this adjunction, therefore,
give us morphisms
\begin{align*}
...
The following theorem is well-known (see e.g., \cite[\S 16]{may};
\cite[Theorem I.11.4]{goerss-jardine}).
...
readers should check, e.g., \cite[Section 7]{simpson} or
\cite[Section I.11]{goerss-jardine}).
...
e.g., \cite[Section III.5]{mac-lane}), an $\calM$-enriched category
...
\begin{example}\label{x:poincare-segal}
  ...
  groupoid}; see \cite[Sections 15.2, 15.3]{simpson}. We will denote it
...
Following \cite{schommer-pries-category}, we give the following definition.
...
\begin{example}\label{x:tamsamani-2cat}
  ...
  Details can be found in \cite[Section 4]{tamsamani} (see also
  \cite{lack-paoli}).
\end{example}
...
A proof of this theorem can be found in
 \cite[Theorem 5.13]{schommer-pries-category}.
...
The following definition was given in \cite{tamsamani}.
\begin{definition}
...
topological $n$-types and $n$-groupoids (see \cite[Theorem 8.0]{tamsamani}).
...
\begin{remark}
  ...
  n}$ is the $n$-fold product of simplex categories; see \cite{paoli} for detailed
  discussion.
\end{remark}
...
\end{theorem}
This construction was first suggested by Lurie in \cite{lurie-cobordism}; an
improved version, fixing some shortcomings of Lurie's, is given in
\cite{calaque}. Note, however, that both references define $(\infty, n)$
