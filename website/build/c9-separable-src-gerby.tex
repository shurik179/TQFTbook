...
the reader to \cite[Chapter 10]{pierce} for details.
...
\begin{remark}
  ...
  unique, see \cite[Theorem 3.1]{aguiar}.
\end{remark}
...
\begin{remark}
  ...
  semisimple algebras is given in \cite[Theorem 2.14]{lauda-pfeiffer}.
\end{remark}
...
\begin{figure}[ht]
  \includegraphics{figures/c9-fig01.svg}
...
coefficients in $M$ (see, e.g., \cite[Section 11.1]{pierce}):
\begin{align*}
...
\begin{exercise}\label{ec:invariants-dual}
  Let $A$ be an algebra, and let $M$ be a finite-dimensional $A$-bimodule.
  Recall that in this case, the dual vector space $M^*$ also has a canonical
  structure of $A$-bimodule, see \ecref{ec:dual-module-frob}.
  \begin{enumerate}
    \item Show that we have canonical isomorphisms
      $(M^*)_A\simeq (M^A)^*$, $(M^*)^A\simeq (M^A)_*$.
    \item Assume in addition that $A$ is a symmetric Frobenius algebra.
      Show that the following formula defines a non-degenerate bilinear
      pairing
        \begin{align*}
          z(A)\otimes A/[A,A]&\to \kk\\
           z\otimes [a]&\mapsto \eps(za).
        \end{align*}
        Deduce from this that the following formula defines a non-degenerate
        symmetric bilinear form on $A/[A,A]$:
        $$
        ([a],[b])=\eps(\sum ax_ibx^i)
        $$
        where $x_i, x^i$ are dual bases in $A$ with respect to Frobenius
        pairing.
    \item Show that if $A$ is a  symmetric Frobenius algebra, then $z(A)$ is
      automatically  a commutative Frobenius algebra with the same counit.
  \end{enumerate}
\end{exercise}
