...
the gauge theory with a finite gauge group, also known as the Dijkgraaf--Witten
model. This theory was introduced by Dijkgraaf in his thesis
\ocite{dijkgraaf-thesis}*{Section 8.5} (see also \ocite{DW}*{Section 6}). In
...
\ocite{freed-quinn} and \ocite{teleman-five-lectures}.
...
\end{equation}
(The reason why this is in quotes will be discussed shortly.) For readers
...
\begin{definition}\label{d:G-bundle}\index{G-bundle@$G$-bundle}
...
technical; one could also use a left action and get an equivalent definition.
...
If we choose a point $\tilde x\in P_x$, we can identify $P_x$ with $G$ by
$g\mapsto \tilde x g$; this gives an identification $\Aut(P_x)\isoto G$
(since any bijection $G\to G$ which commutes with the right action of $G$
is given by left multiplication by some element of $G$). Thus, a pair $(P,
\tilde x)$ gives rise to a monodromy map $\rho \colon \pi_1(M,x)\to G$. It
is easy to see that conversely, for connected $M$, any group homomorphism
$\pi_1(M,x)\to G$ defines a $G$-bundle $P$ together with a choice of a base
point $\tilde x \in P_x$. In other words, we have the following lemma.
...
this lemma as follows: for connected $M$, the groupoid $\tBun_G(M)$
of based $G$-bundles is equivalent to the set
$\Hom(\pi_1(M,x), G)$ (considered as a discrete groupoid: the only morphisms
are identity morphisms).

If we change the choice of $\tilde x$, replacing it by $\tilde x g$ (for the
same $P$), then it is easy to see that the monodromy map
$\rho\colon \pi_1(M,x)\to G$ is conjugated by $g^{-1}$. Thus, we get the
following result.
...
\begin{definition}\label{d:action-groupoid}
...
\begin{example}\label{x:bundles_s1}
  Let $M=S^1$. Then $\tBun_G(S^1)\simeq G$, and $\Bun_G(S^1)\simeq G\q G$,
  where $G$ acts on itself by conjugation. In particular, there is
...
$$
  Z_G(N)=\text{vector space generated by isomorphism classes of
    $G$-bundles on $N$}
$$
and, for a cobordism $N_0\cob{M}N_1$, to define
$$
...
is meaningless: as discussed in \chref{c:categories},
two objects in a category (in this case, two bundles) are rarely
...
\begin{equation}\label{e:count_bung}
  |\Bun_G(M)|=\frac{1}{|G|} |\tBun_G(M)|
\end{equation}
where we denote by $|\tBun_G(M)|$ the number of isomorphism classes of
...
whose proof we leave to the reader: if $X$ is a finite set with an action of a
finite group $G$, then
$$
...
\begin{example}\label{x:bun-circle}
  ...
  $$
  \frac{1}{|\Aut(P_g)|}= \frac{|[g]|}{|G|}
  $$
  where $[g]\subset G$ is the conjugacy class containing $g$.
\end{example}

More generally, we see that for a closed connected manifold $M$, we have
\begin{equation}\label{e:dw_closed}
...
\begin{example}\label{x:action-groupoid}
  ...
  action of $G$ is free.
\end{example}
The above formulas for counting elements in ``sets with automorphisms'' (also
known as groupoids) have a long history; in particular, they have been
extensively used in physics when discussing Feynman diagrams. For a nice
discussion of this, see \ocite{baez-dolan-finite-sets}, or the blog post
\ocite{baez-counting} by John Baez. In particular, we can count how many finite
sets there are --- try doing this yourself!


Similar ideas can also be used, e.g., to define the Euler characteristic of
an orbifold (see \ocite{caramello}*{Section 2.4}). (Note that there are several
...
\begin{example}\label{x:dw-invariants}\par\noindent
  \begin{enumerate}
    ...
    \item Let $M=N\times I$ be the cylinder over $N$.
      ...
      expected.
    \item Let $M$ be the cylinder over $N$ considered as a cobordism $N\sqcup
...
$[P_g]$. By the general theory (\thref{t:2d-classification2}), $A$ must have the
...
\begin{corollary}\label{c:dw-center}
...
of $G$ is isomorphic to a direct sum of copies of the $V_i$. By a well-known
result of representation theory of finite groups,
$\C[G]\simeq \bigoplus_i \End(V_i)$, which implies
\begin{equation}\label{e:dw_projectors}
  A=\C[G]^G=\bigoplus_i \C e_i,
\end{equation}
where $e_i$ are the projectors onto irreducible representations:
$e_i|_{V_j}=\de_{ij}\id_{V_j}$; thus, $e_i e_j = \de_{ij}e_i$.

\begin{remark}\label{r:characters}
  It follows from the orthogonality relations for characters that
  $$
  ...
  were first introduced in \ocite{frobenius-characters}: in modern terminology,
  they were defined as elements of the center of the group algebra such that
  $\chi_i\chi_j = \de_{ij}c_i\chi_i$. The statement that they are traces in
...
multiplicity $d_i=\dim V_i$, \eqref{e:dw_eps} gives
$$
  \eps(e_i) = \la_i = \frac{(\dim V_i)^2}{|G|^2}.
$$
...
On the other hand, by \eqref{e:dw_closed},
$$
  Z_G(\Si_g) =\frac{|\Hom(\pi_1(\Si_g), G)|}{|G|}.
$$
The structure of the fundamental group $\pi_1(\Si_g)$ is well-known. For
$g=0$ this group is trivial, and for $g\ge 1$ it is generated by elements
$a_i, b_i$, $i=1, \dots, g$, with a single relation $\prod_i [a_i,b_i]=1$,
where $[a_i,b_i] = a_ib_ia_i^{-1}b_i^{-1}$ is the group commutator. Thus,
for $g\ge 1$, we get the following identity:
\begin{equation}\label{e:dw_genusg}
...
In particular, for $g=1$ \eqref{e:dw_genusg} gives
$$
...
see \ocite{frobenius-characters}.
...
well known; note, however, that for a nontrivial finite group $G$, the space
$BG$ is necessarily infinite-dimensional.)
...
Thus, the definition of the DW invariant of a closed $n$-manifold can
...
\begin{equation}\label{e:DW-integral}
  Z_G^\om (M)= \int_{[M, BG]}\<\ga^* \om, [M]\> D\ga.
\end{equation}
One should think of $\<\ga^* \om, [M]\>$ as an analog of the term
$e^{iS[\ph]}$ in the path integral.
...
known that for a finite group $G$ and $n\ge 1$, $H^n(BG, \R)=H^n(G,\R)=0$ (here
