...
representative $X_i$ for each $i\in \OC$. We denote by $E$ the set of oriented
 ...
 called a combinatorial manifold. Our main reference is \ocite{balsam-kirillov}.
...
If $F$ is a tetrahedron, then this coincides with the definition in \ocite{barrett}; if $\calC$ is the category of representations 
...
In the special case of a triangulated manifold, this coincides with the construction in \ocite{barrett}.
...
The proofs for this theorem can be found in \ocite{balsam-kirillov}.
...
The proof to this theorem can also be found in \ocite{balsam-kirillov}.

TODO: Requires verification


 It's natural to ask if this construction can be extended to a fully extended TQFT. 
 Balsam and Kirillov's state-sum construction  3-2-1 extends the Turaev-Viro 
 theory. In principal, the same construction could have 
 been straighforwardly extended down to a point. But to show that it defines a 
 fully extended theory, one needs a symmetric monoidal ($\infty$,3) functor. 
 This however requires showing that 0 dimensional data is fully dualizabile in 
 $\Tens$ . And all the equivalences between 1-morphisms should hold upto 
 2-morphisms, and ( compositions of ) 2-morphisms can be further equivalent 
 upto 3-morphisms. This leads to an inhumane web of coherence relations. 
 The extension to 0 manifold clearly requires a more workable definition of a 
 weak 3 category.
...
(see \firef{f:book}). 
\begin{figure}[ht]
	\begin{align*}&\figscale{0.7}{figures/book0.pdf}
...
$$
\frac{1}{\DD}\sum_{j,k\in \OC}d_{j}d_{k}\tzPairingVII
=
\sum_{i\in \OC}d_{i}\tzPairingIII
$$
...
FIXME: stringnet model of $Z(N)$ for closed surface $N$, following 
\ocite{kirillov-stringnet}.


In this section we look at a physics model which, in a certain limit, is described 
by Turaev--Viro TQFT. This model was introduced  by Levin-Wen \ocite{levin-wen}, 
...
general proof for this correspondence can be found in \ocite{kirillov-stringnet}. 
...
oriented illustrations of local relations can be found in the original paper \ocite{levin-wen}.
...
The proof and the precise statement can again be found in \ocite{kirillov-stringnet}.
...
\ocite{petit}*{Theorem 4.2} for a proof.
...
It is worth commenting here that Turaev--Viro TQFTs based on $\vect_G$, with $G$ a finite abelian group,
 ...
 quantum double model, which have potential applications in topological quantum computing. A famous example
  is Kitaev's toric code \ocite{kitaev}, which is based on $G=\Z_2$.


%%%%%%%%%%%%%%%%%%%%
\section{Comparison with cobordism hypothesis}
