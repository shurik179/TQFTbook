...
\ocite{fhk}. See also \ocite{oeckl}. Generalization to arbitrary symmetric
monoidal 2-categories is discussed in \ocite{davidovich}.
...
\begin{equation}\label{e:epsF}
  \begin{aligned}
    \eps_F\colon A^{\otimes \ov{\del F}}&\to \kk\\
                a_1\otimes \dots\otimes a_k&\mapsto \<a_1, \dots,
                a_k\>=\eps(a_1\dots a_k).
  \end{aligned}
\end{equation}
...
Thus, by taking the tensor product of $\eps_F$ over all 2-cells $F$, we
get a linear map
$$
  \eps_M = \bigotimes_F \eps_F \colon A^E\to \kk.
$$
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item
      ...
      $\eps_F w^{-1}_v$ and thus, does not
...
If $I$ is an interval, then $Z_A(I)\simeq A$ is an $A$-bimodule; thus, if we
consider $I$ as a cobordism $\bullet^+\to \bullet^+$, then $Z_A(I)$ is a
morphism $A\to A$ in $\Alg$. We can also consider $I$ as a cobordism
...
\end{theorem}
\begin{proof}
  Proof of the first two parts is an easy
  modification of the proof of \thref{t:fhk1} and is left to the reader.

  For the last part, it suffices to consider $M=S^1\times I$ considered as a
  cobordism $S^1\sqcup S^1\to \varnothing$. In this case, taking the cell
  decomposition of the cylinder obtained by gluing two opposite sides of
  rectangle together, we see that the corresponding map $Z(M)$ is given by
  \begin{align*}
    Z_A(M)\colon V\otimes V &\to \kk, \qquad V=A/[A,A]\\
    [a_1]\otimes [a_2]&\mapsto \eps(\sum a_1 x_i a_2x^i)
  \end{align*}
  where $x_i, x^i$, as usual, are the dual bases with respect to Frobenius
  pairing. The fact that this pairing is non-degenerate follows from
  \ecref{ec:invariants-dual}.
\end{proof}
...
\begin{theorem}\label{t:latticeTQFT}
  ...
  (non-extended) 2-dimen\-sional TQFT.
\end{theorem}
...
\begin{exercise}\label{ec:lattice-pants}
  Show that $Z_A(\tinycopants)\colon V\otimes V\to V$, where $V=Z_A(S^1)=A/[A,A]$,
  is given by
  $$
...
\begin{equation}\label{e:fhk-state-sum4}
  \begin{aligned}
...
One easily sees that it is a discrete version of connections in $G$-bundles:
without the factor $\frac{1}{|G|^V}$, this counts $G$-bundles on $M$ together
with trivializations at every vertex. Thus, dividing by $|G|^V$ gives
