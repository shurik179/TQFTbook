...
of 2-categories; we refer the reader to \ocite{leinster} for details.
...
Following \ocite{schommer-pries}*{Appendix A.4}, we will call diagrams of the
...
below (borrowed with modifications from \ocite{schommer-pries}) shows a
...
\begin{exercise}\label{ec:exchange_condition}
  ...
  technical; see \ocite{power-pasting}.
\end{exercise}
...
\ocite{leinster}*{Lemma 1.2.4}. It can be viewed as a categorical analog of
...
details can be found, e.g., in \ocite{leinster}*{Section 1.5}.


One can also define the notion of functor between (weak) 2-categories, which we
skip, referring the reader to \ocite{leinster}*{Definition 1.5.8}. (Note: what
we will be using is called ``weak functors'' in \ocite{leinster}; compare with
\deref{d:monoidal_functor}). This allows one to define the notion of equivalence
of weak 2-categories.
...
\begin{example}\label{x:BC}
  ...
  =\calC$, with horizontal composition given by tensor product. It is easy to
  see that this is indeed a 2-category; conversely, every 2-category with only
  one object is obtained in this way. This category is called the
  \emph{delooping}\index{delooping} of $\calC$. Moreover, given monoidal
  categories $\calC$, $\calD$, a monoidal functor $F\colon \calC\to\calD$ is the
  same as a weak functor between weak 2-categories $F\colon \calB\calC\to
  \calB\calD$, see \ocite{leinster}*{Example 1.5.9}.
\end{example}
...
\begin{remark}\label{r:BG}
  ...
  The relation $\calB G\simeq \Pi_1(BG)$ above is just the beginning of a chain
  of relations. For example, let $G$ be a topological
  group (which we assume to be nice enough, e.g. having the topology of a CW
  complex); by \exref{x:fund-group}, its fundamental groupoid $\Pi_1(G)$ is a
  monoidal category, so we can define its delooping $\calB (\Pi_1(G))$ which is
  a weak 2-category. Then we have an equivalence of weak 2-categories
  \begin{equation}
    \calB (\Pi_1(G))\cong \Pi_{\le 2}(BG)
  \end{equation}
  where $BG$ is the classifying space, and $\Pi_{\le 2}$ is the fundamental
  2-groupoid (see \exref{x:2groupoid}).

  This statement has a natural generalization for any $k$, which we will
  discuss in FIXME.
\end{remark}
...
the reader to \ocite{leinster}*{Theorem 1.5.15} for the proof.
...
\begin{remark}\label{r:stictify-3-cat}
  ...
  \ocite{gordon-power-street}) is not equivalent to that of a strict 3-category
...
\ocite{schommer-pries}*{Appendix C}.
...
\begin{itemize}
  ...
  \begin{equation}\label{e:pentagon-2cat}
...
\ocite{schommer-pries}*{Appendix C}; he uses $a$ instead of $\al$ for the
...
\ocite{schommer-pries}. Instead, we will show some examples.
...
\end{itemize}
(see \ocite{etingof-fusion}*{Section 1.8}). Denote by $\Linlf$ the 2-category
...
\end{theorem}
We refer the reader to \ocite{etingof-fusion}*{Section 1.11} for the proof of
