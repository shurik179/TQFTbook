...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Classification of 2d TQFTs}\label{c:2d-classification}
...
result; in some form, it goes back at least to 1989 (see Dijkgraaf's thesis
\ocite{dijkgraaf-thesis}*{Section 3.2}), even though a mathematically rigorous
proof was  not  published until 1996. For us, it will serve as a model example
for all future constructions.

For a two-dimensional TQFT, there is only one connected $1$-dimensional oriented
closed manifold: the circle $S^1$, and any orientation-preserving diffeomorphism
$S^1\to S^1$ is isotopic to the identity. It also seems obvious (and in fact can
be justified using Morse theory, as we discuss below) that any cobordism can be
obtained by gluing pieces diffeomorphic to one of the cobordisms below together
with the cylinder $S^1\times I$:
\begin{center}
  \begin{tikzpicture}
    \pic at (0,0) {pants};
...
There are two approaches. One, used in  \ocite{abrams} and \ocite{kock},
uses the classification of surfaces; using it, one shows that the relations
we have are enough to bring any composition of generators to some canonical
form. This approach does not require much beyond the classification of
surfaces, but it does not generalize to higher dimensions.

There is, however, an alternative approach, based on Morse and Cerf theory.
It is presented in \ocite{moore-segal} and \ocite{freed-bordism}*{Lecture~23},
...
It is immediate from the Morse lemma that a non-degenerate critical point
is isolated; thus, if $M$ is compact and all critical points of $f$ are
non-degenerate, there are only finitely many of them.
...
\begin{definition}\label{d:morse-function3}
  ...
  \begin{itemize}
    ...
    \item all critical points of $f$ are in the interior
      $M\setminus (N_0\cup N_1)$, and all critical points are non-degenerate.
  \end{itemize}
  If, in addition, all critical values of $f$ are distinct and different from
  $a_0$, $a_1$, then $f$ is called \emph{excellent}.
\end{definition}
 Again, one can prove
...
give an explicit description of how the topology of $M_t$ changes when one goes
...
\begin{definition}\label{d:elem-cobordism}\index{elementary cobordism}
  An \emph{elementary cobordism} is a cobordism that admits a Morse function
...
the critical point $p_i$. It follows from the result above that up to
...
In particular, for $n=2$, for an excellent Morse function $f$ each non-critical
level set is a smooth one-dimensional manifold (and thus a finite disjoint union
of circles). Using the Morse lemma, it is easy to see that each connected
elementary cobordism is isomorphic to one of the four cobordisms shown in
Table~\ref{tab:elementary-2cob}. Note that here we follow our convention that
cobordisms go ``top to bottom''; thus, the $y$-axis is directed downward, so
that the minimal value of $f$ is at the top, and the maximal, at the bottom.
\begin{table}[ht]
  \centering
...
path $f_s$, $s\in [0,1]$, in the space of functions, in general this path must
go through some points which are not excellent. This was studied by Cerf in
...
\begin{example}\label{x:morse-x3}
  ...
  Then for $s<0$, the function is excellent with two critical points of indices
  $0$ and $1$ respectively; for $s>0$, it is also excellent, with no critical
  ...
  the function space, connects two excellent Morse functions but goes through a
  non-Morse function $f_0=x^3$. This path describes the annihilation of a pair
  of critical points (or, going in the opposite direction, the birth of a pair
  of critical points).
  ...
  \begin{figure}[ht]
    ...
    \begin{tikzpicture}
      ...
      \end{axis}
    \end{tikzpicture}
    \caption{Birth--death of critical points in the family $f_s=x^3+sx$.}
    \label{fig:x3}
  \end{figure}
...
\begin{example}\label{x:morse-crossing}
  Consider the family of functions $f_s(x)=2x^2-x^4+sx$ on $\R$. Then for values
  of $s$ close to 0, $f_s$ has three critical points, all of them
  non-degenerate: a local minimum at $p_0$ close to 0, and local maxima at
  points $p_1<0$ and $p_2>0$, with critical values $c_1=f_s(p_1)$,
  $c_2=f_s(p_2)$.
  ...
  \begin{figure}[ht]
    ...
    \caption{Crossing of critical values in the family $f_s=2x^2-x^4+sx$.}
    \label{fig:x4-x2}
  \end{figure}
...
\begin{definition}\label{d:morse-good}
  ...
  \begin{itemize}
    ...
    \item {\bf Type $\beta$}: all critical points are non-degenerate, and
      ...
      for some $i\ne j$.
  \end{itemize}
  In a similar way one defines good functions on a cobordism $M$.
\end{definition}
...
Now let us apply the general theory of the previous section to the question
of the classification of 2d TQFTs.

First, choosing an excellent Morse function on a 2d cobordism $M$, we get a
decomposition of $M$ into a composition of elementary cobordisms. Each
elementary cobordism is a disjoint union of a connected elementary cobordism and
several cylinders; and each connected elementary cobordism is isomorphic to one
of the standard cobordisms shown in Table~\ref{tab:elementary-2cob}: pair of
pants, copants, cap, and cup.
...
Note that $m$ and $\Delta$ are symmetric: $m\circ P= m$, where
$P\colon A\otimes A\to A\otimes A$ is the permutation
$P(a\otimes b)= b\otimes a$. Indeed, this follows from the isomorphism of
cobordisms shown below: the standard copants surface admits a
self-diffeomorphism (a $180^\circ$ rotation about its central vertical axis)
that swaps the two input boundary circles and maps the output boundary circle to
itself by a rotation, which is isotopic to the identity.
...
Thus, a choice of an excellent Morse function $f$ on $M$, together with a
choice of an isomorphism $\ph_i$ of each of the connected elementary
cobordisms defined by $f$ with one of the ``standard'' ones, gives us a
presentation of $Z(M)$ as a composition of tensor products of operators $m,
i, \Delta, \eps$ shown in \eqref{e:frobenius-data}, and the identity
operator $\id_A$. Therefore, the structure of a TQFT is completely determined
by these four operators.
...
isotopic to the identity, by \leref{l:tqft-isotopy} its action on $A$ is
trivial: $\ph_*=\id_A$. From this it immediately follows that if $M$ is
topologically a disk, considered as a cobordism $\varnothing \cob{} S^1$ (i.e.,
a cap), then the operator $Z(M)\colon \kk \to A$ does not depend on the choice
of the isomorphism of $M$ with the standard cap; a similar result holds for the
cup.

If $M$ is a cobordism $S^1\sqcup S^1\to S^1$ which is isomorphic to the
standard copants, there are just two isomorphisms up to isotopy; one is
...
$a_0<c<a_1$ such that $[a_0,a_1]$ contains no other critical values, and
consider the (non-elementary) cobordism $M=f_{0}^{-1}([a_0, a_1])$.
...
example, defining the tangent vector $v$ to $S$ by the condition that
$(v, \grad f)$ is a positively oriented frame). The orientation also gives a
cyclic order of edges at each vertex $p_i$. Using the Morse lemma, it is easy to
see that near each critical point, $S$ looks as shown below:
...
\end{lemma}
The proof of this lemma is an elementary combinatorial argument; we leave the
details to the reader. Note that this lemma is about abstract graphs (directed,
with a cyclic order of edges at each vertex), not about their embeddings in the
plane.
...
\begin{figure}[ht]
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Possible shapes of the critical level set $S$. Note that for the last
...
corresponding cobordism is shown below (for the reader's convenience, we also
show it in a more familiar form):
$$
\begin{tikzpicture}[vcenter]
...
Summarizing, we see that the non-trivial Cerf moves (both birth--death and
...
by \eqref{e:frobenius-data}) corresponding to each of the moves. For the last
move (Frobenius), we include two versions; this is not necessary, as either of
them implies the other (see \reref{r:frobenius-two}), but it will be useful
later.
...
(together with the symmetry of $m$ discussed above) are equivalent to the
statement that $m, i$ define on $A=Z(S^1)$ the structure of an associative
algebra with unit. Similarly, coassociativity and counit (together with the
symmetry of $\Delta$) say that $\Delta, \eps$ define on $A$ the structure of a
coassociative coalgebra with counit.
...
A standard way to restate the above theorem is using the notion of a Frobenius
algebra.
\begin{definition}\label{d:frobenius}\index{Frobenius algebra}
  A Frobenius algebra is a vector space $A$ over a field $\kk$ with two
  structures:
  \begin{itemize}
...
\end{theorem}
In the next chapter we will show that cocommutativity follows from
commutativity, so it is enough to require $A$ to be commutative; see
\coref{c:frobenius-commutative}.


\begin{example}\label{x:trivial-2d}
  Consider the trivial example $A=\kk$, with the obvious Frobenius algebra
structure. Then it is easy to see that for any closed 1-manifold $N$, including
the empty one, the corresponding TQFT gives $Z(N)=\kk$ (to be precise,
$Z(N)$ is canonically isomorphic to $\kk$), and for any 2-cobordism $M$,
$Z(M)=\id$.
\end{example}
