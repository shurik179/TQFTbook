...
\begin{definition}\label{d:morse-function3}
  ...
  \end{itemize}
  If, in addition, all critical values of $f$ are distinct and different from $a_0$, $a_1$, 
  then $f$ is called \emph{excellent}.
\end{definition}
 Again, one can prove
...
In particular, for $n=2$, for an excellent  Morse function $f$ each
non-critical level set is a smooth one-dimensional manifold (and thus a
finite union of circles). Using Morse lemma, it is easy to see that  each
connected elementary cobordism is isomorphic to one of the four cobordisms
shown in Table~\ref{tab:elementary-2cob}. Note that here we follow our
convention that cobordisms go ``top to bottom''; thus, the $y$-axis is
directed downward, so that the minimal value of $f$ is at the top, and the
maximal, at the bottom.
\begin{table}[ht]
  \centering
...
\begin{example}\label{x:morse-x3}
  ...
  the function space, connects two excellent Morse functions but goes through a non-Morse
...
\begin{example}\label{x:morse-crossing}
  Consider the family of  functions $f_s(x)=x^2-x^4+sx$ on $\R$. Then for
  values of $s$ close to 0, $f_s$ has three  critical points, all of them
  non-degenerate: local minimum at $p_0$ close to 0, and local maxima at
  points $p_1<0$, $p_2>0$.
...
Now let us apply the general theory of hte previous section to the question
of classification of 2d TQFTs.
...
choice of isomorphism $\ph_i$ of each of the connected elementary
...
operator $\id_A$. Therefore, the structure of TQFT is completely determined
by these 4 operators.
...
with cyclis order at vertices, not about their embeddings in the plane).
...
by \eqref{e:frobenius-data}) corresponding to each of the moves. For the 
last move (Frobenius), we included two versions; this is not necessary, 
as any one of them implies the other, but will become useful later. 
...
structure. Then it is easy to see that for any  1-manifold $N$, including
