...
\section{Data for TQFT with defects}
...
$$ Z^{\text{defect}}\colon \Bord_2(M,\text{Coloring})\to \vect_\kk. $$

Here $\Bord_2(M,\text{Coloring})$ is a 2-bordism decorated with certain extra data given as:
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item The vertices ( also called junctions ) are points where two or more defects meet. For each vertex $u \in D^2$, we need a multilinear map $ \varphi_u$ from cyclic tensor produce of incident bimodules to the field $\kk$. For example, if the junction looks like , then it is assigned a map: $\varphi: \circlearrowleft_{A_1} ( M^1 \otimes_{A_2}  M^2 \otimes_{A_3} M^3 \otimes_{A_4} M^4 ) \rightarrow  \kk$. The notation $\circlearrowleft$ means that the map is invariant under cyclic permutations of defects. 
	\end{itemize}
\end{enumerate}


\section{State sum construction of TQFT with defects}
In this section we construct the functor $Z$ that maps the decorated bordisms to $Vect_{\kk}$ using a cell decomposition. Since the cell decomposition is not a part of the actual defect TQFT data, we will prove that this construction is independent of our choice of cell decomposition.
