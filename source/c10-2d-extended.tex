...
TQFTs. Our exposition is based on Schommer-Pries' thesis \ocite{schommer-pries}.
...
\ocite{schommer-pries}, we call such a manifold ``manifold with faces''.
...
\ocite{schommer-pries}, he uses a structure he calls ``halo''. Both of these
approaches give a weak 2-category, and in fact they give equivalent
2-categories, see \ocite{schommer-pries}*{Proposition 3.40}.

We do not give details of either of these constructions, referring the reader to
\ocite{schommer-pries}; this takes a significant part of his thesis.
...
See \ocite{shulman}, \ocite{schommer-pries}*{Section 3.1.4}.
...
``open-closed'' TQFT, studied in \ocite{lauda-pfeiffer}, which allows
...
e.g., in \ocite{dkr}, \ocite{carqueville}. We will talk about these
approaches later.
...
Following \ocite{pstragowski}, we will call a collection of data
...
\ocite{pstragowski}*{Section 3.2} (Note that in that paper, he considers a more
...
As in \ocite{pstragowski}, we will call a fully dual pair \emph{coherent}
...
the rest of the data intact (see \ocite{pstragowski}*{Theorem 3.16}). Thus, we
...
\begin{example}\label{x:DW-extended}
  ...
  (note normalization!) which matches \eqref{e:dw_eps}. Thus, the
