...
\emph{Notices of the AMS}. We will not be discussing this at all; we will
...
\begin{theorem}\label{t:nerve}
  ...
  define a simplicial set $X_\bullet = N\calC_\bullet$, called the \emph{nerve}
  of $\calC$.
...
fiber product\index{fiber product} $A_1\times_B A_2$ is defined by
$$
...
every commutative diagram of the form shown below, there is a unique map $P\to
A_1\times_B A_2$
\begin{equation*}
  \begin{tikzpicture}
    \node (p) at (0,0) {$P$};
...
\end{align*}
Thus, by the universal property of the fiber product, we have the
\emph{Segal map}
\index{Segal!map}
\begin{equation}\label{e:segal-map}
   \mathbf{s}_n\colon X_n \to
   X_1\times_{X_0} X_1\times_{X_0} X_1 \times\dots \times_{X_0} X_1.
\end{equation}
Maps $p_i$ (and thus, the Segal map $\mathbf{s}_n$) can be defined for any
simplicial set: they correspond to the map $[1]\to[n]\colon 0\mapsto (i-1),
1\mapsto i$ in $\Ddelta$.

\begin{theorem}\label{t:segal1}
  A simplicial set $X$ is a nerve of some category $\calC$ if and only if the
  following condition holds: for every $n\ge
...
Thus, (usual) categories can be described as simplicial sets satisfying the
...
One can also define a \emph{geometric realization
...
To describe properties of these morphisms, recall that a
...
not so. In a similar way, we say that a morphism $f\colon K\to K'$ of
simplicial sets is a weak homotopy equivalence if the induced map of geometric
realizations $|K|\to |K'|$ is a weak homotopy equivalence.

Then we have the following well-known theorem (see, e.g., \ocite{may}*{\S 16};
...
In \thref{t:segal1}, we gave a description of a (usual) category $\calC$
as a simplicial set satisfying an additional Segal condition. In a similar way,
one can describe \emph{enriched categories}, i.e., categories where $\Hom$
spaces are not just sets, but have some additional structure --- a structure of
a vector space, or of a topological space. For example, if we denote by $\Top$
the category of topological spaces with continuous maps, a $\Top$-enriched
category is the following data:
\begin{enumerate}
...
Recall that for a category $\calC$, we introduced its nerve $X_\bullet
=N\calC$ to be the simplicial set defined by \eqref{e:nerve1}. If $\calC$
is a $\Top$-enriched category, then $(N\calC)_n$ is not just a set but a
topological space (since we have not defined any topology on the set of
objects, we consider $X_0=\Obj \calC$ as a topological space with the discrete
topology); thus, $N\calC$ is a simplicial space.
...
\begin{theorem}\label{t:nerve-characterization}
  ...
  \begin{itemize}
    ...
    \item For any $n\ge 1$, the Segal map \eqref{e:segal-map} is a homeomorphism.
  \end{itemize}
\end{theorem}
...
\begin{theorem}\label{t:nerve-enriched}
  ...
  \begin{enumerate}
    ...
    $$
      \mathbf{s}_n\colon X(x_0, \dots, x_n)\to
...
category $\Top$, i.e., a functor $\Ddelta^\op\to \Top$.
%Recall also that a
%continuous map of topological spaces $f\colon X\to Y$ is called a \emph{weak
%homotopy equivalence}\index{weak homotopy equivalence} if for every $n\ge0$, it
%defines an isomorphism of homotopy groups $\pi_n(X)\simeq \pi_n(Y)$.
%In particular, if $f$ is a homotopy equivalence, then it is automatically
% a weak
%homotopy equivalence. By a famous result of Whitehead, if $X,Y$ are CW
%complexes, then the converse is also true; for general topological spaces, it
%is not so.
...
\begin{definition}\label{d:segal-enriched}
  ...
  \thref{t:nerve-enriched}) which satisfies the following conditions:
    \begin{enumerate}
      ...
      \item For any $x_0, \dots, x_n \in S$, $n\ge 1$, the Segal map
      $$
         \mathbf{s}_n\colon X(x_0, \dots, x_n)\to
                X(x_0, x_1)\times \dots\times X(x_{n-1}, x_n)
      $$
...
\begin{example}\label{x:tamsamani-2cat}
  ...
  This is not obvious; details can be found in \ocite{tamsamani}*{Section 4}
  (see also \ocite{lack-paoli}).
\end{example}
...
is called the \emph{1-truncation} of $\calC$; alternatively, it is also called
the \emph{homotopy category}\index{homotopy category} of $\calC$ (compare with
 \deref{d:hC}, where we defined the 1-truncation for a weak 2-category).
...
theory of weak $n$-categories, Grothendieck's homotopy hypothesis holds: the
functor $X\mapsto \Pi_{\leq n}(X)$ gives rise to an equivalence of homotopy
theories of topological $n$-types and $n$-groupoids (see
\ocite{tamsamani}*{Theorem 8.0}). Thus, this theory is a reasonable candidate
...
\begin{remark}
  ...
  n}$ is the $n$-fold product of simplex categories; see \ocite{paoli} for a detailed
  discussion.
\end{remark}
...
$n$-categories (however, to the best of our knowledge, it has not been explicitly
done yet).
